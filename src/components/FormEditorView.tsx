...
import {
  ...
  FORM_SECTIONS, cloneSections, getAllEnumFields,
...
import {
  ...
  Link, Sun, Radio, X, Check, AlertCircle, Loader2, Sparkles, Copy,
} from 'lucide-react';
...
const FIELD_TYPES: { value: FieldType; label: string; icon: string }[] = [
  ...
  { value: 'Photo', label: 'Foto (sección)', icon: '📷' },
  { value: 'Image', label: 'Imagen (campo)', icon: '🖼' },
  { value: 'Show', label: 'Mostrar texto', icon: '💬' },
  { value: 'Ref', label: 'Referencia (auto)', icon: '🔗' },
];
...
export default function FormEditorView() {
  ...
  const handleReset = () => {
    ...
    toast({ title: 'Esquema restaurado', description: 'Se ha restaurado el esquema por defecto' });
  };
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = 'preventivo_schema.json';
    a.click();
    ...
    <div className="max-w-5xl mx-auto p-4 sm:p-6">
      ...
      <div className="flex items-start gap-3 mb-6">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-xl font-bold text-teal-800 flex items-center gap-2">
            ...
            Editor de Formulario Preventivos
          </h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            Personaliza las secciones, campos, opciones y lógica condicional
          </p>
        </div>
        <div className="flex gap-2 shrink-0">
          <Button variant="outline" size="sm" onClick={handleExport} className="border-teal-200">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <Card className="border-teal-100">
          <CardContent className="p-3 text-center">
            <p className="text-2xl font-bold text-teal-800">{schema.length}</p>
            <p className="text-xs text-muted-foreground">Secciones</p>
          </CardContent>
        </Card>
        <Card className="border-blue-100">
          <CardContent className="p-3 text-center">
            <p className="text-2xl font-bold text-blue-800">{totalFields}</p>
      ...
      {isCustomized && (
        <div className="mb-4 bg-violet-50 border border-violet-200 rounded-xl p-3 flex items-center gap-2">
          ...
          <span className="text-sm text-violet-700">Esquema personalizado activo</span>
        </div>
      )}
      ...
      <div className="space-y-3">
        {schema.map((section, sectionIdx) => {
          ...
          return (
            <Card key={section.key} className="border shadow-sm">
              {/* Section Header */}
              <div
                className="flex items-center gap-2 p-3 cursor-pointer hover:bg-teal-50/50 transition-colors rounded-t-xl"
                onClick={() => toggleSection(section.key)}
              >
                <GripVertical className="h-4 w-4 text-gray-300" />
                {isExpanded ? <ChevronDown className="h-4 w-4 text-teal-600" /> : <ChevronRight className="h-4 w-4 text-teal-600" />}
                <span className="font-semibold text-sm text-teal-800 flex-1">{section.label}</span>
                ...
                <div className="flex items-center gap-0.5 ml-2" onClick={e => e.stopPropagation()}>
                  ...
                  <Button variant="ghost" size="icon" className="h-7 w-7"
                    ...
                    {section.visible === false ? <EyeOff className="h-3.5 w-3.5 text-gray-400" /> : <Eye className="h-3.5 w-3.5 text-teal-500" />}
                  </Button>
                </div>
              </div>
                ...
                <CardContent className="pt-0 pb-3 px-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className="w-full h-8 text-xs text-teal-600 hover:bg-teal-50 border border-dashed border-teal-200"
                  ...
                  {section.subsections.map((sub, subIdx) => (
                    <div key={sub.key} className="mt-3 ml-2 border-l-2 border-teal-200 pl-3">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-xs font-bold text-teal-700 uppercase tracking-wide">{sub.label}</span>
                        <Badge variant="secondary" className="text-[10px] bg-teal-50 text-teal-600">
                          {sub.fields.length}
                        </Badge>
                        ...
                        <div className="ml-auto flex gap-0.5">
                          <Button variant="ghost" size="icon" className="h-6 w-6"
                            ...
                            {sub.visible === false ? <EyeOff className="h-3 w-3 text-gray-400" /> : <Eye className="h-3 w-3 text-teal-500" />}
                          </Button>
                          <Button variant="ghost" size="icon" className="h-6 w-6"
                      ...
                      <div className="space-y-1">
                        ...
                        <Button
                          ...
                          className="w-full h-7 text-xs text-teal-600 hover:bg-teal-50 border border-dashed border-teal-200"
                  ...
                  <Button
                    variant="ghost" size="sm"
                    className="w-full h-8 mt-3 text-xs text-violet-600 hover:bg-violet-50 border border-dashed border-violet-200"
        ...
        <Button
          variant="outline"
          className="w-full h-12 border-dashed border-teal-300 text-teal-600 hover:bg-teal-50"
      ...
      {/* Add Section Dialog */}
      {showAddSection && (
        <AddSectionDialog onClose={() => setShowAddSection(false)} />
      )}

      {/* Add Subsection Dialog */}
      {showAddSubsection && (
        <AddSubsectionDialog sectionKey={showAddSubsection} onClose={() => setShowAddSubsection(null)} />
      )}
    </div>
  );
}
...
function FieldRow({ field, sectionKey, subsectionKey, onEdit, onMoveUp, onMoveDown, onToggleVisibility, onDelete, onToggleRequired }: {
  ...
  return (
    <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border transition-colors group ${
      field.visible === false ? 'bg-gray-50 border-gray-200 opacity-60' : 'bg-white border-gray-100 hover:border-teal-200'
    }`}>
      <GripVertical className="h-3.5 w-3.5 text-gray-300 shrink-0" />

      {/* Type badge */}
      <span className="text-[10px] font-mono bg-teal-50 text-teal-700 px-1.5 py-0.5 rounded shrink-0 border border-teal-100">
        {typeInfo?.icon || '?'}
      </span>
      ...
      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={onToggleVisibility}>
          {field.visible === false ? <EyeOff className="h-3 w-3 text-gray-400" /> : <Eye className="h-3 w-3 text-teal-500" />}
        </Button>
...
// Field Editor Dialog
// ==========================================
function FieldEditorDialog({ field, sectionKey, subsectionKey, allSections, onClose }: {
      ...
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-4 mt-2">
          ...
          {(fieldType === 'Enum' || fieldType === 'EnumList') && (
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                />
                <Button size="sm" onClick={addOption} className="h-8 bg-teal-600 hover:bg-teal-700 text-white">
          ...
          <div className="flex justify-end gap-2 pt-2 border-t">
            <Button variant="outline" onClick={onClose} className="border-teal-200">Cancelar</Button>
...
// Add Field Dialog
// ==========================================
function AddFieldDialog({ sectionKey, subsectionKey, onClose }: {
  sectionKey: string;
  subsectionKey: string | null;
  onClose: () => void;
}) {
  ...
  return (
    <Dialog open onOpenChange={() => onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="text-teal-800">Nuevo Campo</DialogTitle>
        </DialogHeader>
...
function AddSectionDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Dialog open onOpenChange={() => onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="text-teal-800">Nueva Sección</DialogTitle>
        </DialogHeader>
        <div className="space-y-4 mt-2">
          <div className="space-y-1.5">
            <Label className="text-sm font-medium">Nombre de la sección *</Label>
            <Input value={label} onChange={e => setLabel(e.target.value)} placeholder="Ej: Datos Generales" className="h-10" autoFocus />
          </div>
...
// Add Subsection Dialog
// ==========================================
function AddSubsectionDialog({ sectionKey, onClose }: { sectionKey: string; onClose: () => void }) {
  const schemaStore = useSchemaStore();
  const { toast } = useToast();
  const [label, setLabel] = useState('');
  ...
  return (
    <Dialog open onOpenChange={() => onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="text-teal-800">Nueva Subsección</DialogTitle>
        </DialogHeader>
        <div className="space-y-4 mt-2">
          <div className="space-y-1.5">
            <Label className="text-sm font-medium">Nombre de la subsección *</Label>
            <Input value={label} onChange={e => setLabel(e.target.value)} placeholder="Ej: Máquina Principal" className="h-10" autoFocus />
          </div>
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={onClose} className="border-teal-200">Cancelar</Button>
            <Button onClick={handleAdd} className="bg-teal-600 hover:bg-teal-700 text-white">
