...
// ─── Dynamic Map Import ─────────────────────────────────────────
// Must be dynamic because Leaflet requires `window`

// We store leaflet module reference for use in cluster icon creation
...
// We'll use a custom marker approach since react-leaflet's Marker doesn't play
// well with dynamic imports. We create a wrapper component.
function MapMarkers({ centros, onMarkerClick }: { centros: CentroData[]; onMarkerClick: (c: CentroData) => void }) {
  const [L, setL] = useState<typeof import('leaflet') | null>(null);
  const [components, setComponents] = useState<{ Marker: any; Popup: any; useMap: any; Tooltip: any } | null>(null);
  ...
  useEffect(() => {
    if (components?.useMap) {
      // no-op, just making sure map is accessible
    }
  }, [components]);

  if (!L || !components) return null;

  const { Marker, Popup, Tooltip } = components;

  // Create custom icons per prioridad
  const getIcon = (prioridad: string, tipo: string) => {
    const config = PRIORIDAD_CONFIG[prioridad] || PRIORIDAD_CONFIG.P3;
    const size = tipo === 'INDOOR' ? 28 : 32;
      ...
      html: `
        ...
        ">
          <div style="
            ...
            box-shadow: 0 2px 8px rgba(0,0,0,0.35), 0 0 0 1px ${config.color}40;
            display: flex;
            align-items: center;
            justify-content: center;
...
function FitBounds({ centros }: { centros: CentroData[] }) {
  const [useMap, setUseMap] = useState<any>(null);
  ...
  if (!useMap) return null;
  return <FitBoundsInner centros={centros} useMap={useMap} />;
}

function FitBoundsInner({ centros, useMap }: { centros: CentroData[]; useMap: any }) {
  const map = useMap();
  const fitted = useRef(false);

  useEffect(() => {
    if (centros.length > 0 && !fitted.current) {
...
export default function MapView() {
  ...
  useEffect(() => {
    setMounted(true);
    // Pre-load leaflet for cluster icons
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-8rem)] gap-0 overflow-hidden">
      {/* ─── Sidebar ──────────────────────────────────────────────── */}
      <div className={`${sidebarOpen ? 'w-full lg:w-[380px]' : 'w-0 lg:w-0'} flex-shrink-0 transition-all duration-300 overflow-hidden border-r border-border bg-background`}>
        <div className="w-[380px] h-full flex flex-col">
          ...
          <div className="p-4 border-b space-y-3">
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">
                  OnTower / Cellnex — {filteredCentros.length} de {centros.length} centros
                </p>
              </div>
              <Button variant="ghost" size="icon" onClick={() => setSidebarOpen(false)} className="hidden lg:flex">
            ...
            <div className="relative">
              ...
              />
              {searchText && (
                <Button variant="ghost" size="icon" className="absolute right-1 top-1/2 -translate-y-1/2 h-6 w-6" onClick={() => setSearchText('')}>
                  <X className="h-3 w-3" />
                </Button>
              )}
            </div>
            ...
            {/* Active filters */}
            {(filterProvincia !== 'all' || filterPrioridad !== 'all' || filterTipo !== 'all' || searchText) && (
              <div className="flex flex-wrap gap-1.5">
                {searchText && (
                  <Badge variant="secondary" className="text-[10px] gap-1 pr-1">
                    &quot;{searchText}&quot;
                    <X className="h-2.5 w-2.5 cursor-pointer" onClick={() => setSearchText('')} />
                  </Badge>
                )}
                {filterProvincia !== 'all' && (
                  <Badge variant="secondary" className="text-[10px] gap-1 pr-1">
                ...
                <Button variant="ghost" size="sm" className="h-5 text-[10px] text-muted-foreground px-1" onClick={clearFilters}>
                  Limpiar
                </Button>
              </div>
            )}
          ...
          <div className="px-4 py-2 border-b bg-muted/30">
            <div className="grid grid-cols-4 gap-1.5 text-center">
              ...
              {Object.entries(stats.byPrioridad).sort().map(([key, val]) => (
                <div key={key}>
                  <p className="text-sm font-bold" style={{ color: PRIORIDAD_CONFIG[key]?.color || '#888' }}>{val}</p>
            ...
            <div className="flex gap-2 mt-1.5">
              {Object.entries(stats.byTipo).sort().map(([key, val]) => (
                <div key={key} className="flex items-center gap-1">
                  ...
                  <span className="text-[9px] text-muted-foreground">{key}: {val}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <ScrollArea className="flex-1">
            <div className="p-2 space-y-1">
              {filteredCentros.map(centro => {
                ...
                const isHovered = listHover === centro.codigo;
                ...
                return (
                  <button
                    ...
                    onMouseEnter={() => setListHover(centro.codigo)}
                    onMouseLeave={() => setListHover(null)}
                    onClick={() => handleFlyToCentro(centro)}
                  >
                    <div className="flex items-start gap-2">
                      <div
                        className="flex-shrink-0 w-3 h-3 rounded-full mt-1 ring-2 ring-white dark:ring-gray-800"
                      ...
                      <Navigation className="h-3.5 w-3.5 text-muted-foreground/40 flex-shrink-0 mt-0.5" />
                    </div>
                  </button>
                );
              })}
          ...
          <div className="p-3 border-t bg-muted/20">
            <p className="text-[10px] font-semibold text-muted-foreground mb-1.5 uppercase tracking-wider">Leyenda</p>
            <div className="flex flex-wrap gap-x-3 gap-y-1">
              {Object.entries(PRIORIDAD_CONFIG).map(([key, cfg]) => (
                <div key={key} className="flex items-center gap-1.5">
                  <div className="w-3 h-3 rounded-full ring-1 ring-white dark:ring-gray-800" style={{ backgroundColor: cfg.color }} />
                  <span className="text-[10px] text-muted-foreground">{key}</span>
                </div>
              ))}
              <div className="flex items-center gap-1.5 ml-2 pl-2 border-l">
                <div className="w-3 h-3 rounded-full ring-1 ring-white dark:ring-gray-800" style={{ backgroundColor: TIPO_CENTRO_COLORS.INDOOR }} />
                <span className="text-[10px] text-muted-foreground">Indoor</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded-md ring-1 ring-white dark:ring-gray-800" style={{ backgroundColor: TIPO_CENTRO_COLORS.OUTDOOR, transform: 'rotate(45deg)', width: 10, height: 10 }} />
                <span className="text-[10px] text-muted-foreground">Outdoor</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 relative">
        ...
        <div className="absolute top-3 right-3 z-[1000] flex flex-col gap-2">
          ...
          {!sidebarOpen && (
            <Button
              variant="default"
              size="icon"
              className="h-10 w-10 shadow-lg bg-background text-foreground hover:bg-muted border border-border"
          ...
          {/* Map style toggle */}
          <div className="flex flex-col bg-background rounded-lg border border-border shadow-lg overflow-hidden">
            <Button
          ...
          <Button
            variant="default"
            size="icon"
            className="h-9 w-9 shadow-lg bg-background text-foreground hover:bg-muted border border-border"
            onClick={() => {
              // Force fit bounds by remounting
              setFlyTo(null);
        ...
        <div className="absolute top-3 left-3 z-[1000]">
          <Badge variant="outline" className="bg-background/90 backdrop-blur-sm border-border shadow-sm text-xs px-3 py-1.5">
            <MapPin className="h-3 w-3 mr-1 text-primary" />
            {filteredCentros.length} centros en vista
          </Badge>
        </div>
          ...
          >
            ...
            <MarkerClusterGroup
              ...
              iconCreateFunction={(cluster: any) => {
                ...
                // Calculate the dominant prioridad in cluster
            ...
            <FitBounds centros={filteredCentros} />
          </MapContainer>
        )}
      </div>
