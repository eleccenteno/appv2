...
import { CENTROS_SECTIONS, cloneSections } from '@/lib/centros-schema';
import { FormSection, FormField, VisibilityCondition } from '@/lib/preventivo-schema';

const STORAGE_KEY = 'ec_centros_custom_schema';
...
interface CentrosSchemaState {
  ...
  updateField: (sectionKey: string, subsectionKey: string | null, fieldKey: string, updates: Partial<FormField>) => void;
  addField: (sectionKey: string, subsectionKey: string | null, field: FormField, index?: number) => void;
  ...
  // Condition mutations
  addCondition: (sectionKey: string, subsectionKey: string | null, fieldKey: string, condition: VisibilityCondition) => void;
  removeCondition: (sectionKey: string, subsectionKey: string | null, fieldKey: string, conditionId: string) => void;
  updateCondition: (sectionKey: string, subsectionKey: string | null, fieldKey: string, conditionId: string, updates: Partial<VisibilityCondition>) => void;
...
function mutateField(
  ...
  fieldKey: string,
  mutator: (field: FormField, arr: FormField[], idx: number) => void
): FormSection[] {
...
export const useCentrosSchemaStore = create<CentrosSchemaState>((set, get) => ({
  customSchema: null,
  isCustomized: false,
  ...
  getSchema: () => {
    ...
    return customSchema || CENTROS_SECTIONS;
  },
  ...
  moveField: (sectionKey, subsectionKey, fieldKey, direction) => {
    ...
    const newSchema = schema.map(section => {
      ...
      const swapInArray = (fields: FormField[]) => {
